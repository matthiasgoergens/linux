 ...
 *
 * Copy of runlist-holes/rdcheck.c from the series' reproducer tag, changed to
 * classify a final partial block too (needed for a file size that is not a
 * multiple of 4 KiB).
 *
 ...
 * is not a full-length success, and the block classifications (including a
 * final partial block, compared byte by byte) are printed
...
static enum st classify(const uint8_t *b, size_t len, uint64_t id, uint64_t off)
{
	int zero = 1, ok = 1;

	for (size_t i = 0; i < len; i++) {
		uint64_t w = (id << 56) | ((off + i) & ~7ULL);

		if (b[i])
			zero = 0;
		if (b[i] != (uint8_t)(w >> (8 * ((off + i) & 7))))
			ok = 0;
	}
...
int main(int argc, char **argv)
{
	...
	if (!strcmp(argv[3], "pread")) {
		...
		printf("READ off=%llu ret=%zd errno=%d (%s)\n",
		       ...
		       r < 0 ? strerror(errno) : "-");
		if (r == BLK)
			note(off, classify(buf, BLK, id, off));
		else
			note(off, S_ERR);
		emit(off + BLK);
	} else {
		while (off < size) {
			...
			for (ssize_t i = 0; i < r; i += BLK)
				note(off + i, classify(buf + i, r - i < BLK ? r - i : BLK,
						       id, off + i));
			off += r;
		}
		emit(off);
